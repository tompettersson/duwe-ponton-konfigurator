...
export function InteractionManager() {
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (event: MouseEvent) => {
      ...
      // CRITICAL FIX: Calculate click coordinates from actual click event, not stale pointer move
      const rect = gl.domElement.getBoundingClientRect();
      ...
      if (currentHoveredCell) {
        console.log('🎯 CLICK using hoveredCell (Single Source of Truth):', currentHoveredCell);
        ...
        console.log('❌ No hoveredCell available for click - fallback to raycast');
        
        // Fallback: Only if no hover state available (should be rare)
        raycaster.current.layers.set(LAYERS.GRID);
        ...
        if (gridIntersects.length > 0) {
          ...
          if (intersection) {
            ...
            console.log('🎯 FALLBACK CLICK with fresh raycast coordinates:', gridPos);
